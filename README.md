...
| 📺 **B站** | 搜索 + 视频详情（bili-cli，无需登录） | 视频下载（bilix）、字幕（OpenCLI） | 告诉 Agent「帮我配 B站」 |
...
- "把这个 B站视频下载下来" → `bilix get_video`（无需登录可下 480P，登录后 1080P+）
...
| B站 | [bili-cli](https://github.com/public-clis/bilibili-cli)（搜索/读取）+ [bilix](https://github.com/HFrost0/bilix)（下载） | OpenCLI ▸ 搜索 API（字幕） | yt-dlp 被 B站风控 412 封死（2026-06 实测），bili-cli 无登录可搜可读，bilix 负责视频下载（不触发风控） |
...
### 🔌 其他项目通过 MCP Server 调用

Agent Reach 自带一个 MCP Server，其他 AI 项目（Claude Code、Cursor、自定义 Agent 等）可以通过 MCP 协议复用它的平台能力（当前支持 B 站搜索、详情、热门、视频下载）。

#### 第 1 步：安装 Agent Reach + 依赖

```bash
# 安装 Agent Reach
pip install https://github.com/Panniantong/agent-reach/archive/main.zip
# 或锁定版本：pip install https://github.com/Panniantong/agent-reach/archive/v1.5.0.zip

# 安装 MCP SDK（Python）
pip install mcp

# 安装 B 站后端工具
pip install bilibili-cli   # 搜索、详情、热门（无需登录）
brew install bilix         # 视频下载（macOS；其他平台见 bilix 文档）
```

#### 第 2 步：在你的项目里配置 MCP

**Claude Code / Claude Desktop**（`settings.json` 或 `claude_desktop_config.json`）：
...
async def main():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # 调用 B 站搜索
            result = await session.call_tool("bilibili_search", {"query": "AI agent", "n": 5})
            print(result)
...
| 工具 | 说明 | 后端 | 是否需登录 |
|------|------|------|:---:|
| `bilibili_search` | 搜索 B站视频/用户 | bili-cli | ❌ |
| `bilibili_hot` | B站热门视频 | bili-cli | ❌ |
| `bilibili_video_info` | 视频详情、字幕可用性 | bili-cli | ❌ |
| `bilibili_download` | 下载视频/音频/字幕/弹幕/封面 | bilix | ❌（登录后解锁 1080P+）|

MCP Server 走 stdio 协议（标准输入/输出 JSON-RPC），不需要网络端口，配置好即可用。
...
[OpenCLI](https://github.com/jackwener/opencli) · [twitter-cli](https://github.com/public-clis/twitter-cli) · [rdt-cli](https://github.com/public-clis/rdt-cli) · [xiaohongshu-mcp](https://github.com/xpzouying/xiaohongshu-mcp) · [xhs-cli](https://github.com/jackwener/xiaohongshu-cli) · [bili-cli](https://github.com/public-clis/bilibili-cli) · [bilix](https://github.com/HFrost0/bilix) · [yt-dlp](https://github.com/yt-dlp/yt-dlp) · [Jina Reader](https://github.com/jina-ai/reader) · [Exa](https://exa.ai) · [mcporter](https://github.com/nicobailon/mcporter) · [feedparser](https://github.com/kurtmckee/feedparser) · [linkedin-scraper-mcp](https://github.com/stickerdaniel/linkedin-mcp-server)
