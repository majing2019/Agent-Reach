...
PLATFORM_SPECS = [
    ...
    # ===== E-commerce platforms =====
    {
        "name": "JD.com",
        "domains": [".jd.com"],
        "cookies": None,  # grab all cookies as string
        "config_key": "jd",
    },
    {
        "name": "Taobao",
    ...
    },
    {
        ...
        "cookies": None,
        "config_key": "aliexpress",
    },
    {
        "name": "Shopee",
        "domains": [".shopee.com", ".shopee.sg", ".shopee.tw", ".shopee.co"],
        "cookies": None,
        "config_key": "shopee",
    },
    {
        "name": "Lazada",
        "domains": [".lazada.com", ".lazada.sg", ".lazada.co"],
        "cookies": None,
        "config_key": "lazada",
    },
]
...
def configure_from_browser(browser: str, config) -> List[Tuple[str, bool, str]]:
    ...
    # ===== E-commerce platforms =====
    _configure_ecommerce(extracted, results_list)

    return results_list


def _configure_ecommerce(extracted: dict, results_list: list) -> None:
    """Pipe extracted e-commerce cookies to ecommerce-cli configure."""
    import subprocess

    # Check if ecommerce-cli is installed
    ecom_available = False
    try:
        result = subprocess.run(
            ["ecommerce-cli", "--version"],
            capture_output=True, encoding="utf-8", errors="replace",
            timeout=5,
        )
        ecom_available = result.returncode == 0
    except Exception:
        pass

    ecom_platforms = [
        "jd", "taobao", "tmall", "pinduoduo", "goofish",
        "amazon", "ebay", "walmart", "bestbuy", "etsy", "target",
        "aliexpress", "shopee", "lazada",
    ]

    for platform in ecom_platforms:
        if platform not in extracted:
            continue

        cookie_str = extracted[platform].get("cookie_string", "")
        if not cookie_str:
            continue
        ...
        if ecom_available:
            try:
                subprocess.run(
                    ["ecommerce-cli", platform, "configure", "--cookie", cookie_str],
                    capture_output=True, encoding="utf-8", errors="replace",
                    timeout=15,
                )
                results_list.append((
                    ...
                    True,
                    f"{n_cookies} cookies → ecommerce-cli profile",
                ))
            except Exception as e:
                results_list.append((
                    f"E-Commerce: {platform}",
                    False,
                    f"Failed to configure ecommerce-cli: {e}",
                ))
        else:
            results_list.append((
                f"E-Commerce: {platform}",
                False,
                f"Found {n_cookies} cookies, but ecommerce-cli not installed. "
                f"Install: pipx install ecommerce-cli",
            ))
